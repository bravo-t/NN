    ...
    for(int epoch=1;epoch<=epochs;epoch++) {
        ...
        for(int iteration=0;iteration<iterations;iteration++) {
            ...
            if ((epoch % 1000 == 0 && iteration == 0) || verbose) {
                printf("INFO: Epoch %d, learning rate: %f, data loss: %f, regulization loss: %f, total loss: %f, training accuracy: %f\n",
                    epoch, learning_rate, data_loss, reg_loss, data_loss+reg_loss, accu);
            }
...
void assignSlaveArguments(SlaveArgs* args,
    ...
    args->tmp = tmp;
    args->alpha = alpha;
    args->learning_rate_addr = learning_rate;
